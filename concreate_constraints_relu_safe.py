...
#
# We scan the MNIST test set for a "non-black" image (one with at least 50 nonzero
# pixels). If none is found, we default to index 0. The chosen image is then
# resized to 32×32 and normalized to [0,1]. Each pixel, weight, and bias value
# is converted into an 8-bit posit bit pattern. The output file contains the
# layer-by-layer algebraic constraints for formal verification.
# ------------------------------------------------------------------------------
...
##############################################################################
# 2. Define LeNet with variable names
##############################################################################
def LeNet(x):
    mu = 0
    sigma = 0.1
...
def float_to_posit8_bits(val: float) -> str:
    """Casts 'val' to posit8 then returns the 8-bit binary representation."""
    p = posit(val)  # e.g. np.posit8
    return np.binary_repr(p.view(np.uint8), width=8)

##############################################################################
# 5. Load one "non-black" MNIST image, resize to 32×32, normalizes to [0,1]
##############################################################################
...
img_resized = np.array(pil_resized, dtype=np.float32) / 255.0
...
##############################################################################
# 6. Convert W/B arrays to 8-bit posit strings for each element
##############################################################################
encoded_params = {}
for var_name, arr in weights_and_biases.items():
    shape = arr.shape
    # We'll build an array of strings, same shape
...
def generate_conv_constraints(w_4d, b_1d, in_shape, layer_name, index_start, prev_layer=None):
    """
    w_4d: shape=(fh,fw,in_d,out_d), each element is 8-bit posit bitstring
    b_1d: shape=(out_d,), 8-bit posit bitstrings
    in_shape: (H,W,Depth)
    prev_layer: if None => raw input
    """
    constraints = []
    node_count = 0
    ...
    for out_c in range(out_d):
        ...
        for i in range(in_shape[0] - fh + 1):
            for j in range(in_shape[1] - fw + 1):
                ...
                for fh_ in range(fh):
                    for fw_ in range(fw):
                        for d_ in range(in_d):
                            ...
                            # If prev_layer is None, we multiply by the raw image bits
                ...
                idx += 1
                # ReLU: z_{layer_name}_{i}_{j}_{out_c}
                constraints.append(
                    f"{idx}: z_{layer_name}_{i}_{j}_{out_c} = if y_{layer_name}_{i}_{j}_{out_c} >= 00000000 "
                    f"then y_{layer_name}_{i}_{j}_{out_c} else 00000000"
                )
                idx += 1
                node_count += 1
    ...
    for d in range(out_d):
        for i in range(out_h):
            for j in range(out_w):
                # we take max of the four z_{prev_layer_name}_... outputs
                expr = (
                    f"max("
...
def generate_flatten_constraints(prev_layer_name, in_shape, out_layer_name, index_start):
    """
    Convert m_{prev_layer_name}_{i}_{j}_{d} into a 1D index:
      m_{out_layer_name}_k = m_{prev_layer_name}_{i}_{j}_{d}
    This matches how flatten() merges (H,W,Depth) into a single dimension.
    """
    constraints = []
    node_count = 0
    idx = index_start
...
def generate_fc_constraints(w_2d, b_1d, in_shape, layer_name, index_start, prev_layer, prev_is_pool=False):
    """
    w_2d: shape=(in_size,out_size), each is 8-bit posit bits
    b_1d: shape=(out_size,), each is 8-bit posit bits
    in_shape: e.g. (400,) => in_size=400
    layer_name: e.g. "fc1", "fc2"
    prev_layer: e.g. "flatpool2" (if from pool) or "fc1" (if from previous FC)
    prev_is_pool: if True => use m_{prev_layer}_{k}, else => z_{prev_layer}_{k}
    """
    constraints = []
    node_count = 0
    ...
    # We'll either multiply by m_{prev_layer}_{k} or z_{prev_layer}_{k}
    ...
    for out_i in range(out_size):
        ...
        idx += 1
        constraints.append(
            f"{idx}: z_{layer_name}_{out_i} = if y_{layer_name}_{out_i} >= 00000000 "
            f"then y_{layer_name}_{out_i} else 00000000"
        )
        idx += 1
        node_count += 1
...
##############################################################################
# 8. Build the constraints layer-by-layer
##############################################################################
all_constraints = []
idx = 1
total_nodes = 0

# 1) conv1 => output shape (28,28,6)
...
# 7) fc2 => shape (120,84)
# Now the previous layer is fc1 (a fully-connected layer), so we reference z_fc1_...
c, n, idx = generate_fc_constraints(
...
# 8) fc3 => shape (84,10)
# Similarly, prev_layer="fc2" => reference z_fc2_...
c, n, idx = generate_fc_constraints(
...
##############################################################################
output_file = "fully_posit_constraints_with_relu.txt"
