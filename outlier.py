...
##############################################################################
# 7. Standard constraint generators
##############################################################################
...
def generate_flatten_constraints(prev_layer_name, in_shape, out_layer_name, index_start):
    ...
    for i in range(H):
        for j in range(W):
            for d in range(D):
                constraints.append(f"{idx}: m_{out_layer_name}_{k} = m_{prev_layer_name}_{i}_{j}_{d}")
                idx+=1
                node_count+=1
                k+=1
    return constraints,node_count,idx
...
##############################################################################
# 7B. Quadruple final layer + outlier detection => but final lines go last
##############################################################################
def generate_fc3_quadruple_anomaly(w_2d, b_1d, index_start, prev_layer_name):
    """
    1) replicate fc3 => fc3A..fc3D
    2) outlier detection => safeA_i..safeD_i
    3) final_i = max(safeA_i, safeB_i, safeC_i, safeD_i)
    BUT we store final_i lines separately and append last.
    """
    constraints=[]
    final_lines=[]  # we'll keep final_0..9 lines here
    node_count=0
    idx=index_start
    ...
    def fc3_replica(repName, w_2d, b_1d, idx):
        ...
        for i in range(out_size):
            ...
            idx+=1
            localCount+=1
        return lines, localCount, idx

    # replicate final layer 4 times
    linesA, countA, idxA = fc3_replica("fc3A", w_2d, b_1d, idx)
    constraints.extend(linesA)
    node_count += countA
    idx=idxA

    linesB, countB, idxB = fc3_replica("fc3B", w_2d, b_1d, idx)
    constraints.extend(linesB)
    node_count += countB
    idx=idxB

    linesC, countC, idxC = fc3_replica("fc3C", w_2d, b_1d, idx)
    constraints.extend(linesC)
    node_count += countC
    idx=idxC

    linesD, countD, idxD = fc3_replica("fc3D", w_2d, b_1d, idx)
    constraints.extend(linesD)
    node_count += countD
    idx=idxD

    # thresholds
    hiThresh = "00010111"  # ~0.35
    loThresh = "10000000"  # negative or NaR

    # outlier detection
    for i in range(out_size):
        # replicate A
        constraints.append(f"{idx}: outlierA_high_{i} = if y_fc3A_{i} >= {hiThresh} then 00000001 else 00000000")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: outlierA_low_{i} = if y_fc3A_{i} >= {loThresh} then 00000000 else 00000001")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: sum_outlierA_{i} = outlierA_high_{i} + outlierA_low_{i}")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: outlierA_{i} = if sum_outlierA_{i} >= 00000001 then 00000001 else 00000000")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: safeA_{i} = if outlierA_{i} >= 00000001 then 00000000 else y_fc3A_{i}")
        idx+=1; node_count+=1

        # replicate B
        constraints.append(f"{idx}: outlierB_high_{i} = if y_fc3B_{i} >= {hiThresh} then 00000001 else 00000000")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: outlierB_low_{i} = if y_fc3B_{i} >= {loThresh} then 00000000 else 00000001")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: sum_outlierB_{i} = outlierB_high_{i} + outlierB_low_{i}")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: outlierB_{i} = if sum_outlierB_{i} >= 00000001 then 00000001 else 00000000")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: safeB_{i} = if outlierB_{i} >= 00000001 then 00000000 else y_fc3B_{i}")
        idx+=1; node_count+=1

        # replicate C
        constraints.append(f"{idx}: outlierC_high_{i} = if y_fc3C_{i} >= {hiThresh} then 00000001 else 00000000")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: outlierC_low_{i} = if y_fc3C_{i} >= {loThresh} then 00000000 else 00000001")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: sum_outlierC_{i} = outlierC_high_{i} + outlierC_low_{i}")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: outlierC_{i} = if sum_outlierC_{i} >= 00000001 then 00000001 else 00000000")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: safeC_{i} = if outlierC_{i} >= 00000001 then 00000000 else y_fc3C_{i}")
        idx+=1; node_count+=1

        # replicate D
        constraints.append(f"{idx}: outlierD_high_{i} = if y_fc3D_{i} >= {hiThresh} then 00000001 else 00000000")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: outlierD_low_{i} = if y_fc3D_{i} >= {loThresh} then 00000000 else 00000001")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: sum_outlierD_{i} = outlierD_high_{i} + outlierD_low_{i}")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: outlierD_{i} = if sum_outlierD_{i} >= 00000001 then 00000001 else 00000000")
        idx+=1; node_count+=1
        constraints.append(f"{idx}: safeD_{i} = if outlierD_{i} >= 00000001 then 00000000 else y_fc3D_{i}")
        idx+=1; node_count+=1

        # We'll store final_i = max(...) lines in final_lines, so they appear last:
        lineFinal = f"{idx}: final_{i} = max(safeA_{i}, safeB_{i}, safeC_{i}, safeD_{i})"
        final_lines.append(lineFinal)
        idx+=1
        node_count+=1

    # after all outlier lines for i=0..9, we append the final lines
...
# 8.8 quadruple final layer => shape(84,10), outlier detection, final lines appended last
fc3C,fc3N,idx = generate_fc3_quadruple_anomaly(
...
# 9. Write constraints
##############################################################################
output_file = "fully_posit8_constraints_4xFC3_anomaly_finalLast.txt"
...
print("Final 10 lines => final_0..final_9. No outlier var is in last 10 lines.")
